...
void bubblesort(Iter first, Iter last){
    while(first != last){
        Iter jump  = first;
        for(Iter j = first, i = j++; j != last; i = j++){ // (previous, next)
            if(*j > *i) { // Basically, j > j + 1, meaning if (previous > next) SWAP Increasing Order
                jump = j;
                std::swap(*j,*i);
            }
        }
...
int main(){
	...
	/*print(v1); //prints out same 5000 random numbers of type int for each vector of integers
    print(v2);
    print(v3);
    print(v4);*/
