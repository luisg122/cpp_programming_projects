...
bool Gaussian::div(const Gaussian &other){				//imperfect code for divisibilty 
	int x,y,c,d,z;
	bool isPrime = true;
	...
	if(isPrime){									//if isPrime != 0, therefore isPrime == 1(true)
		if(x != 0){
			if((y % x) == 0){
...
//--------------------------------------------------------------------------------------------------------	
	...
	Gaussian m(4,5),n(14,-3);	//true
	
	bool bb = (m|n);		
    cout << bb;	   				//1, therefore True
