...
//method for finding #of prime numbers given some range9
...
bool Prime::ptest(uint n){
  if(n <= 1) return false;
  int sq = sqrt(n);
  for(uint i = 2;i <= sq;++i){		//initialized from 2 because 2 is the FIRST accepted prime number
...
int main(){
  Prime P(pow(10,6));//10^6
  cout<<P.is_prime(79)<<endl;
  cout<<P.count(1,pow(10,6))<<endl;
}
