...
class MyVector{
  ...
  public:
    ...
    MyVector(int *p,uint n):sz(n),cap(n),data( n ? new int[n]:nullptr){
      //if(n) { if n==0, std::copy will skip it.
          std::copy(p,p+n,data);
      //}
    }
    ...
    void reserve(uint n){
        if(n>cap){
            int * tmp = new int[n];
            std::copy(data,data+sz,tmp);//if sz==0, skip copy. http://en.cppreference.com/w/cpp/algorithm/copy
            delete[] data;
            data=tmp;
            cap=n;
        }
...
MyVector& MyVector::operator=(const MyVector & other){
    if(this!=&other){
        if(other.sz>cap){
            ...
            std::copy(other.data,other.data+other.sz,tmp);//sz>cap>=0
            delete[] data;
            data=tmp;
            cap=sz=other.sz;
        }
